...
const {axiosInstance} = useContext(AppContext)
 ...
 useEffect(() => {
  axiosInstance.get(`/getallregistrations?page=${page}&limit=${limit}`)
...
return (
 <>
    {
        reviewId && (
            <ReviewRegistration id={reviewId} setAdmissionRefresh={setAdmissionRefresh} admissionRefresh={admissionRefresh}/>
            )
        }
